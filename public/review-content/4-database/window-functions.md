...
<details>
<summary><b>Q: Sự khác biệt giữa ROW_NUMBER và RANK là gì?</b></summary>

`ROW_NUMBER()` đánh số thứ tự liên tục tăng dần (1, 2, 3, 4). `RANK()` đánh số thứ tự có nhảy hạng nếu trùng giá trị (1, 2, 2, 4).
</details>

<details>
<summary><b>Q: Làm thế nào để lấy giá trị hàng trước đó trong SQL?</b></summary>

Sử dụng window function `LAG(col, offset)` kết hợp mệnh đề `OVER (ORDER BY ...)` để lấy dữ liệu dòng trước đó.
</details>

<details>
<summary><b>Q: Bạn có thể dùng window function trong mệnh đề WHERE không?</b></summary>

Không trực tiếp dùng được vì WHERE chạy trước window function. Phải bọc câu query chứa window function vào một Subquery/CTE rồi lọc ở ngoài.
</details>

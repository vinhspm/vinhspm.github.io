...
<details>
<summary><b>Q: Điều gì xảy ra khi tất cả HikariCP connection đang được dùng và có request mới?</b></summary>

Request mới sẽ bị block và đưa vào hàng đợi chờ kết nối trống. Nếu đợi quá thời gian `connectionTimeout` (mặc định 30 giây) mà vẫn không có kết nối nào được giải phóng, HikariCP sẽ ném ra `SQLTransientConnectionException`.
</details>

<details>
<summary><b>Q: Tính maximumPoolSize thế nào khi deploy 5 app instance?</b></summary>

Tổng số kết nối từ tất cả 5 instance không được vượt quá giới hạn tối đa mà Database Server hỗ trợ (`max_connections`). Công thức cơ bản: `5 * maximumPoolSize < max_connections`. Cần chừa lại một số kết nối cho các công cụ quản trị khác.
</details>

<details>
<summary><b>Q: Connection leak detection là gì và cấu hình thế nào?</b></summary>

Là cơ chế phát hiện các kết nối được lấy ra nhưng không được đóng lại (close) để trả về pool. Cấu hình bằng thuộc tính `leakDetectionThreshold` (mili-giây), nếu kết nối bị giữ lâu hơn thời gian này, HikariCP sẽ log cảnh báo kèm theo stack trace để debug.
</details>

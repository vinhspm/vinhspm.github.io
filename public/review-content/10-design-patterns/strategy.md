...
<details>
<summary><b>Q: Khi nào thay if-else bằng Strategy?</b></summary>

Khi số lượng nhánh điều kiện if-else quá nhiều (hoặc có xu hướng tăng liên tục trong tương lai) và logic xử lý bên trong mỗi nhánh độc lập, phức tạp.
</details>

<details>
<summary><b>Q: Làm thế nào để chọn strategy lúc runtime trong Spring?</b></summary>

Inject toàn bộ strategy con vào một Map thông qua Spring: `Map<String, MyStrategy>`. Khi chạy, dựa vào tham số đầu vào để gọi `map.get(strategyName)` để lấy ra chiến lược tương ứng để chạy.
</details>

<details>
<summary><b>Q: Mối quan hệ giữa Strategy và Open/Closed Principle?</b></summary>

Strategy là hiện thân trực tiếp của OCP: cho phép mở rộng hệ thống bằng cách tạo thêm class chiến lược mới (Open for extension) mà hoàn toàn không cần sửa đổi code logic lựa chọn chiến lược ban đầu (Closed for modification).
</details>
